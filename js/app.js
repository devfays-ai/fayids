// ══════════════════════════════════════════════════════════
//  FayID Portfolio — Main App
//  Vercel · Discord OAuth (popup + redirect fallback)
// ══════════════════════════════════════════════════════════

// ── Config ────────────────────────────────────────────────
// IMPORTANT: In Discord Developer Portal → OAuth2 → Redirects, add BOTH:
//   https://fayid.qzz.io/callback
//   https://fayid.qzz.io/callback.html
// Use the one WITHOUT .html as the primary (matches URL bar better on mobile).
const CONFIG = {
  DISCORD_CLIENT_ID: '',
  // Use /callback (no extension) — matches Vercel rewrite rule
...
function handleDiscordLogin() {
  if (!CONFIG.DISCORD_CLIENT_ID) {
    showToast('⚙️ Discord not configured — demo login active', 'warn');
    demoLogin();
    return;
  }

  const state = (crypto.randomUUID?.() ?? Math.random().toString(36).slice(2) + Date.now().toString(36));
  ...
  // Always use redirect flow (most reliable on mobile browsers)
  window.location.href = `https://discord.com/api/oauth2/authorize?${params}`;
}

// ── Popup postMessage handler (fallback for desktop popup flow) ──
...
async function handleSessionCallback() {
  ...
  // Clean up immediately so refreshing doesn't re-trigger
...
// ── Also handle ?code= in URL (some redirect setups) ───────────
...
// ── Demo login (no Discord configured) ──────────────────────────
function demoLogin() {
  setUser({
    id:         'demo_' + Date.now(),
    username:   'DemoUser',
...
function startTypewriter() {
  ...
  const raw   = window.I18n ? I18n.t('hero.taglines') : null;
  const lines = Array.isArray(raw)
    ? raw
    : ['ROBLOX DEVELOPER', 'DISCORD BOT BUILDER', '3D MODELER', 'LUA SCRIPTER'];
...
function initReveal() {
  ...
  document.querySelectorAll(
    '.skill-card, .terminal-card, .about-info, .section-header, .rating-stats-panel, .rating-form-panel'
  ).forEach(el => {
...
document.addEventListener('DOMContentLoaded', async () => {
  // 1. Fetch server config (Discord client ID)
  await fetchConfig();

  // 2. Handle OAuth callbacks — both flows
  await handleSessionCallback(); // redirect flow via sessionStorage
  await handleURLCallback();     // direct ?code= in URL fallback
  ...
  // 5. Typewriter (slight delay so i18n can load first)
